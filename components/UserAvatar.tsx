...
export function UserAvatar({ name, color, size = "md" }: UserAvatarProps) {
  ...
  return (
    <div
      className={`${dim} rounded-full flex items-center justify-center font-semibold text-white ring-2 ring-white dark:ring-gray-950 flex-shrink-0`}
    ...
    <div className="relative" ref={ref}>
      ...
      >
        <div className="flex -space-x-1">
          ...
          {extra > 0 && (
            <div className="w-9 h-9 rounded-full bg-gray-200 dark:bg-gray-700 ring-2 ring-white dark:ring-gray-950 flex items-center justify-center text-xs font-semibold text-gray-600 dark:text-gray-300">
              +{extra}
            </div>
          )}
        </div>
        <span className="text-sm text-gray-500 dark:text-gray-400">{users.length} online</span>
      </button>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-800 py-2 z-50">
          <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide px-4 pb-2">
            In this session
          </p>
          <ul>
            {users.map((u) => (
              <li key={u.user_id} className="flex items-center gap-3 px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                <UserAvatar name={u.user_name} color={u.user_color} size="sm" />
                <span className="text-sm text-gray-800 dark:text-gray-200 font-medium truncate">{u.user_name}</span>
